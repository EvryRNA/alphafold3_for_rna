...
class BarHelper:
    ...
    @staticmethod
    def normalize_metrics(df, desc_metrics: List = DESC_METRICS):
        metrics, datasets = df["Metric"].unique(), df["Dataset"].unique()
        for metric in metrics:
            mask = (
                (df["Metric"] == metric)
                & df["Metric (value)"].notna()
                & np.isfinite(df["Metric (value)"])
            )
    ...
    def viz_dataset(self, dataset: List):
        """Plot the polar distribution for a dataset."""
        width, height = 1000, 700
        df = self.df[self.df["Dataset"].isin(dataset)]
        df = (
            df[["Metric", "Model", "Metric (value)"]]
        ...
        save_path = os.path.join(
            "data", "plots", "figures", "bar", "_".join(dataset) + ".png"
        )
